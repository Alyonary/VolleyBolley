...
class TestTourneyModel:
    ...
    @pytest.mark.parametrize(
        'tourney', [(lf('tourney_thai_ind')), (lf('tourney_thai_team'))]
    )
    def test_create_tourney_teams(self, tourney: Tourney) -> None:
        assert tourney.teams.count() == tourney.maximum_teams
...
class TestTourneyAPI:
    def test_url_detail(
        self,
        api_client_thailand: APIClient,
        tourney_for_args: List[int],
    ) -> None:
        """The test of the availability of the url of tournament detail."""
        url: str = reverse('api:tournaments-detail', args=tourney_for_args)
    ...
    def test_tourney_create(
        ...
        currency_type_thailand: Any,
        game_levels_light: Any,
        player_thailand: Player,
        is_individual: bool,
        maximum_players: int,
        maximum_teams: int,
        result_max_players: int,
        result_max_teams: int,
    ) -> None:
        """
        ...
        """
        url: str = reverse('api:tournaments-list')
        ...
        created_tourney: Tourney = Tourney.objects.last()
    ...
    def test_for_invite_creation(
        ...
        tourney: Tourney,
        api_client_thailand: APIClient,
        bulk_create_registered_players: List[Player],
    ) -> None:
        url: str = reverse(
            'api:tournaments-invite-players', args=(tourney.id,)
        )
        player_ids: List[int] = [
            player.id for player in bulk_create_registered_players
        ]
        for p in bulk_create_registered_players:
            print(p.user.username, p.is_registered, 'CHECKP')
        data: Dict[str, Any] = {'players': player_ids}
        response = api_client_thailand.post(url, data, format='json')
        assert response.status_code == status.HTTP_201_CREATED
        assert GameInvitation.objects.count() == len(player_ids)
        for user in player_ids:
            invite = tourney.event_invites.filter(invited=user).first()
            assert invite is not None

    def test_create_invitation_wrong_level(
        self,
        tourney_thai_ind: Tourney,
        api_client_thailand: APIClient,
        player_thailand_female_pro: Player,
    ) -> None:
        levels: List[str] = [
            level.name for level in tourney_thai_ind.player_levels.all()
        ]
        assert player_thailand_female_pro.rating.grade not in levels
        url: str = reverse(
            'api:tournaments-invite-players', args=(tourney_thai_ind.id,)
        )
        data: Dict[str, Any] = {
            'players': [
                player_thailand_female_pro.id,
            ]
        }
        response = api_client_thailand.post(url, data, format='json')
    ...
    def test_invite_all_not_registered_player(
        self,
        api_client_thailand: APIClient,
        tourney_thai_ind: Tourney,
        bulk_create_not_registered_players: List[Player],
    ) -> None:
        """
        Ensure that invitation cannot be sent to unregistered players.
        """
        url: str = reverse(
            'api:tournaments-invite-players', args=(tourney_thai_ind.id,)
        )
        player_ids: List[int] = [
            player.id for player in bulk_create_not_registered_players
        ]
        response = api_client_thailand.post(
            url, {'players': player_ids}, format='json'
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert GameInvitation.objects.count() == 0

    def test_invite_some_not_registered_player(
        self,
        api_client_thailand: APIClient,
        tourney_thai_ind: Tourney,
        bulk_create_registered_players: List[Player],
        ...
        """
        Ensure that invitation can be sent to registered players even if some
        of the players in the list are not registered.
        """
        url: str = reverse(
            'api:tournaments-invite-players', args=(tourney_thai_ind.id,)
        )
        ...
        not_registered_player_ids: List[int] = [
            player.id for player in bulk_create_not_registered_players
        ]
        player_ids: List[int] = (
            registered_player_ids + not_registered_player_ids
        )
        response = api_client_thailand.post(
            url, {'players': player_ids}, format='json'
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        for user in registered_player_ids:
            invite = tourney_thai_ind.event_invites.filter(
                invited=user
            ).first()
            assert invite is None

    def test_accept_invite_when_team_full(
        self,
        api_client_thailand_pro: APIClient,
        tourney_thai_team: Tourney,
        player_thailand_female_pro: Player,
        bulk_create_registered_players: List[Player],
    ) -> None:
        """
        Ensure invite cannot be accepted if the team is already full.
        """
        team: TourneyTeam = tourney_thai_team.teams.first()
        ...
        response = api_client_thailand_pro.post(
            url, {'team_id': team.id}, format='json'
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.json()['is_joined'] is False
        assert player_thailand_female_pro not in team.players.all()

    def test_accept_invite_after_tourney_started(
        self,
        api_client_thailand_pro: APIClient,
        tourney_thai_ind: Tourney,
        player_thailand_female_pro: Player,
        ...
        """
        Ensure invite cannot be accepted if the tournament has already started.
        """
        tourney_thai_ind.start_time = now() - timedelta(hours=1)
        tourney_thai_ind.save()
        GameInvitation.objects.create(
            host=tourney_thai_ind.host,
            invited=player_thailand_female_pro,
            content_object=tourney_thai_ind,
        )
        team: TourneyTeam = tourney_thai_ind.teams.first()
        url: str = reverse(
    ...
    def test_successful_add_to_team(
        self,
        api_client_thailand_pro: APIClient,
        tourney_thai_team: Tourney,
        player_thailand_female_pro: Player,
        ...
        """
        Ensure player is successfully added to the team after accepting invite.
        """
        GameInvitation.objects.create(
            host=tourney_thai_team.host,
            invited=player_thailand_female_pro,
...
class TestTourney:
    ...
    def test_for_tourney_joining(
        ...
        request_data: dict[str, int] = {'team_id': tourney.teams.last().id}
        response = api_client_thailand_pro.post(
    ...
    def test_team_tourney_joining_with_wrong_data(
        ...
        request_data: dict[str, int] = {
            'team_id': tourney_thai_ind.teams.last().id
        }
        response = api_client_thailand_pro.post(
            url, request_data, format='json'
        )
        ...
        # wrong key
        request_data_wrong: dict[str, int] = {
            'wrong_key': tourney_thai_team.teams.last().id
        }
        response = api_client_thailand_pro.post(
            url, request_data_wrong, format='json'
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert player_thailand_female_pro not in tourney_thai_team.players
        # no invitation
        request_data: dict[str, int] = {
            'team_id': tourney_thai_team.teams.last().id
        }
        response = api_client_thailand_pro.post(
            url, request_data, format='json'
        )
        ...
        assert player_thailand_female_pro not in tourney_thai_team.players
        # already paticipate
        team: TourneyTeam = tourney_thai_team.teams.last()
        another_team: TourneyTeam = tourney_thai_team.teams.first()
        ...
        request_data: dict[str, int] = {'team_id': another_team.id}
        response = api_client_thailand_pro.post(
            url, request_data, format='json'
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        ...
        request_data: dict[str, int] = {'team_id': team.id}
        response = api_client_thailand_pro.post(
            url, request_data, format='json'
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_for_tourney_invitation_declining(
        ...
        url: str = reverse(
            'api:tournaments-delete-invitation', args=(tourney_thai_ind.id,)
        )
    ...
    def test_filtering_queryset(
        ...
        assert (
            response.data['tournaments'][0]['tournament_id']
            == tourney_thai_ind.id
        )
...
class TestTourneySerializers:
    ...
    def test_tourney_detail_serializer(
        ...
        response = api_client_thailand.get(
            reverse('api:tournaments-detail', args=(tourney.id,))
        )
        ...
        assert response_data['tournament_id'] == tourney.id
        assert response_data['is_individual'] == tourney.is_individual
        assert response_data['message'] == tourney.message
        assert (
            parse_datetime(response_data['start_time']) == tourney.start_time
        )
        assert parse_datetime(response_data['end_time']) == tourney.end_time
        assert response_data['gender'] == tourney.gender
        assert response_data['price_per_person'] == tourney.price_per_person
        assert (
            response_data['currency_type']
            == tourney.currency_type.currency_type
        )
        assert response_data['payment_type'] == tourney.payment_type
        assert response_data['payment_account'] == tourney.payment_account
        assert response_data['maximum_players'] == tourney.max_players
        assert response_data['maximum_teams'] == tourney.maximum_teams
    ...
    def test_tourney_short_serializer(
        ...
        assert response_data['message'] == tourney_thai_ind.message
        assert (
...
class TestTourneyFiltering:
    def test_preview_filtering(
        self, api_client_thailand, create_custom_tourney, tourney_thai_ind
    ):
    ...
    def test_archive_filtering(
        self, tourney_thai_ind, create_custom_tourney, player_thailand
    ):
    ...
    def test_invites_filtering(
        self, tourney_thai_ind, tourney_thai_team, player_thailand_female_pro
    ):
        assert Tourney.objects.count() == 2
